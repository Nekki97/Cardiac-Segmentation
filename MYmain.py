                ...
                for layers in layers_arr:
                    ...
                    for perc in data_percs:
                        for seed in seeds:
                            ...
                            print("TEST DATA SIZE", sum([len(test_images)*test_images[i].shape[0] for i in range(len(test_images))]))
                            print("****************************************************************************************************************")
                            ...
                            if whichmodel == 'DenseUNet':
                                model = DenseUNet(input_size, loss_func)

                            es = keras.callbacks.EarlyStopping(monitor='val_loss', min_delta=0.0, patience=30, verbose=1,
                                                               mode='min', baseline=0.5, restore_best_weights=False)
                            ...
                            for i in test_images:
                                print(i.shape, "mask_prediction")
                                mask_prediction.append(model.predict(i, verbose=1))

                            rounded_pred = threshold(mask_prediction, 0.5, 0.5)
                            rounded_pred = np.squeeze(rounded_pred, 0)
                            mask_prediction = np.array(mask_prediction)
                            mask_prediction = np.squeeze(mask_prediction, 0)

                            print(test_images[0].shape)
                            print(test_masks[0].shape)
                            print(rounded_pred.shape)
                            print(mask_prediction.shape, "mask_prediction2")
                            ...
                            for i in range(len(test_masks)):
                                for s in range(test_masks[i].shape[0]):
                                    dice.append(getdicescore(test_masks[i][s, :, :, 0], rounded_pred[s, :, :, 0]))
                            ...
                            for i in range(len(test_images)):
                                for slice in test_images[i]:
                                    all_test_imgs.append(slice)
                                for slice in test_masks[i]:
                                    all_test_masks.append(slice)
                            all_test_imgs = np.array(all_test_imgs, dtype=float)
                            all_test_masks = np.array(all_test_masks, dtype=float)

                            # save_visualisation(test_images, test_masks, mask_prediction, output, median_matthew_coeff, 'matthews', save_dir)
                            save_visualisation(all_test_imgs, all_test_masks, mask_prediction, rounded_pred, median_dice_score, 'dice', save_dir)
